...
module Make (Ip:V1_LWT.IP) = struct
  ...
  let xmit ~ip ~id ?(rst=false) ?(syn=false) ?(fin=false) ?(psh=false)
    ...
    printf "Wire.xmit checksum=%04x %s.%d->%s.%d rst=%b syn=%b fin=%b psh=%b seq=\
         %lu ack=%lu options=%s datalen=%d datafrag=%d dataoff=%d olen=%d\n%!" checksum
         (string_of_ip id.local_ip) id.local_port
         (string_of_ip id.dest_ip) id.dest_port
